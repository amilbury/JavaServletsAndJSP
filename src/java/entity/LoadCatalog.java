...
public class LoadCatalog extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String fileName = "/monitors.csv"; // put real file name
        ...
        while (sc.hasNextLine()) {
            ...
            String rr = pieces[4];
            String ips = pieces[5];
            boolean goodIps = false;
            if(ips.equals("yes")){
                goodIps = true;
            }
            double price = Double.parseDouble(pieces[6]);
            Monitor m = new Monitor(partNum, desc, resolution, size, rr, goodIps, price);
            Order o = new Order(m, 0);
            orders.add(o);
        }

        request.getSession().setAttribute("orders", orders);
        
        // forward to JSP to build the results page
